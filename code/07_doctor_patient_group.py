...
def get_doctor_info(url, html):    
    ...
    group_num_org = select.xpath('//p[@class="fl"]//font[@class="orange1 ml5 mr5"]//text()')
    group_num_inf = [i.strip() for i in group_num_org if len(i.strip()) > 0][0]

    
    group_detail_info = {}
    group_detail_org = select.xpath('//ul[@class="clearfix fs pa_friend_list"]//text()')
    group_detail_inf = [i.strip().replace('(','') for i in group_detail_org if len(i.strip()) > 0 and i.strip() != ')']
    group_detail_number = 20 if len(group_detail_inf)//5 >= 20 else len(group_detail_inf)//5
    
    for i in range(group_detail_number):
        sub_info = {}
        sub_info[group_detail_inf[5*i+1]] = int(group_detail_inf[5*i+2])
        sub_info[group_detail_inf[5*i+3]] = int(group_detail_inf[5*i+4])
        group_detail_info[group_detail_inf[5*i]] = sub_info
    
    doctor_info['姓名'] = name_inf[0]
    doctor_info['患友会链接'] = url
    doctor_info['患友会数量'] = group_num_inf
    doctor_info['患友会详情'] = group_detail_info
...
if __name__ == '__main__':
    ...
    patient_group_url_list = read_data(file_path)

    # set store file structure
    save_path = 'C:/Users/lenovo/Desktop/HDF_data/02_data/7_患友会数据/'+ date + '_doctor_patient_group.csv'
    header = ['姓名', '患友会链接', '患友会数量', '患友会详情']
    save_data_header(header, save_path)

    # get doctor info
    count = 0
    for url in patient_group_url_list:
        count += 1
